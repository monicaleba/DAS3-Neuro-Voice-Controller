...
PRIORITY_WORDS = {
    ...
    'ELBOW_UP': ['elbow', 'up', 'forearm', 'in', 'upward'],
    'ELBOW_DOWN': ['elbow', 'down', 'forearm', 'out', 'downward'],
...
class SuperExoControllerGUI:
    ...
    # ─── PHONETIC INTERCEPTOR ─────────────────────────────────────────────
    def sanitize_transcription(self, raw_text):
        """Catches and replaces known ASR hallucinations before reaching the Gatekeeper."""
        # Convert to upper case for robust matching
        clean_text = raw_text.upper()

        # Anatomical Hallucinations
        clean_text = clean_text.replace("FOUR ARM", "FOREARM")
        clean_text = clean_text.replace("FOR ARM", "FOREARM")
        clean_text = clean_text.replace("4 ARM", "FOREARM")
        clean_text = clean_text.replace("HERBO", "ELBOW")
        clean_text = clean_text.replace("HERBOW", "ELBOW")
        clean_text = clean_text.replace("ELBEAU", "ELBOW")
        clean_text = clean_text.replace("ELBO", "ELBOW")
        clean_text = clean_text.replace("ELBOWW", "ELBOW")
        clean_text = clean_text.replace("SHORTER", "SHOULDER")
        clean_text = clean_text.replace("APARAR", "UPPER ARM")
        clean_text = clean_text.replace("AUTUMN", "ARM")
        clean_text = clean_text.replace("ARME RIGHT WORD", "ARM RIGHTWARD")
        clean_text = clean_text.replace("ARM RIGHT WORD", "ARM RIGHTWARD")
        clean_text = clean_text.replace("ARM LEFT WORD", "ARM LEFTWARD")
        clean_text = clean_text.replace("RIGHT WORD", "RIGHTWARD")
        clean_text = clean_text.replace("LEFT WORD", "LEFTWARD")

        # Safety / Command Hallucinations
        clean_text = clean_text.replace("CHEESE", "CEASE")
        clean_text = clean_text.replace("A BOARD", "ABORT")
        clean_text = clean_text.replace("ABOARD", "ABORT")
        clean_text = clean_text.replace("CANCE CANCELL", "CANCEL")
        clean_text = clean_text.replace("FRIES", "FREEZE")
        clean_text = clean_text.replace("G8", "GYRATE")
        clean_text = clean_text.replace("SWEDEN", "SWIVEL")
        clean_text = clean_text.replace("CANCER", "CANCEL")
        clean_text = clean_text.replace("COUNCIL", "CANCEL")
        clean_text = clean_text.replace("BOTH OPERATIONS", "PAUSE OPERATIONS")
        clean_text = clean_text.replace("BOTH OPERATION", "PAUSE OPERATION")

        return clean_text

    # ─── MASTER NLP PIPELINE & HIERARCHICAL GATEKEEPER ────────────────────
    def process_input_text(self, text):
        """Passes text through the Hierarchical Gatekeeper, then the SLM, then dispatches to OpenSim"""
        text_lower = text.lower()
        
        found_stop = False
        found_rest = False
        has_rotate_wrist = False
        other_words_found = set()

        # 1. Scan the sentence for ALL valid keywords
            ...
            for word in words:
                if re.search(r'\b' + re.escape(word.lower()) + r'\b', text_lower):
                    if category == 'STOP':
                        ...
                        found_rest = True
                    elif category == 'ROTATE_WRIST':
                        has_rotate_wrist = True
                    else: 
                        other_words_found.add(word.lower())

        # 2. HIERARCHICAL OVERRIDE LOGIC
        # Tier 1: Absolute Priority (STOP)
        if found_stop:
            self.print_to_terminal("  [GATEKEEPER] EMERGENCY OVERRIDE TRIGGERED -> STOP")
            return "STOP" # Bypasses the SLM completely for instant safety
            
        # Tier 2: Secondary Priority (REST)
        if found_rest:
            self.print_to_terminal("  [GATEKEEPER] OVERRIDE TRIGGERED -> REST")
            return "REST" # Bypasses the SLM completely
            
        # Tier 3: Single-Word Command (ROTATE_WRIST)
        if has_rotate_wrist:
            pass # Gatekeeper allows it to pass to the SLM
            
        # Tier 4: Standard Movement (Requires 2+ valid anatomical keywords)
        elif len(other_words_found) >= 2:
            pass # Gatekeeper allows it to pass to the SLM
        ...
        # 3. SLM Inference (Only runs for Movement and Rotate_Wrist commands)
        ...
        # 4. Translation to Standard Token
    ...
    def _on_text_enter(self, event=None):
        ...
        # Sanitize text from manual entry as well
        clean_text = self.sanitize_transcription(user_input)
        token = self.process_input_text(clean_text)
    ...
    def _process_voice(self, audio_data):
        ...
        try:
            ...
            if final_text:
                self.print_to_terminal(f"  [MIC RAW]: '{final_text}'")
                
                # Sanitize the transcription
                clean_text = self.sanitize_transcription(final_text)
                
                # Only print the cleaned version if it actually changed
                if clean_text != final_text:
                     self.print_to_terminal(f"  [SANITIZED]: '{clean_text}'")
